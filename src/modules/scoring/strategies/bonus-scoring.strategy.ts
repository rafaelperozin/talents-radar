...
@Injectable()
export class BonusScoringStrategy {
  constructor(private readonly weightNormalizer: WeightNormalizer) {}

  calculate(talent: TalentEntity, bonus?: BonusFiltersDto): number {
    if (!bonus) return 0;

    const subScores = this.collectSubScores(talent, bonus);
  ...
  private collectSubScores(
    ...
    type Key = keyof typeof DEFAULT_BONUS_FILTER_WEIGHTS;
    const criteria: [Key, boolean, () => number][] = [
      [
        'technicalSkills',
        !!bonus.technicalSkills?.length,
        () => this.technicalSkillsScore(talent, bonus.technicalSkills!),
      ],
      [
        'softSkills',
        !!bonus.softSkills?.length,
        () => this.softSkillsScore(talent, bonus.softSkills!),
      ],
      [
        'salary',
        bonus.salary !== undefined && bonus.salary > 0,
        () => this.salaryScore(talent, bonus.salary!),
      ],
      [
        'workType',
        !!bonus.workType?.length,
        () => this.workTypeScore(talent, bonus.workType!),
      ],
      [
        'seniority',
        !!bonus.seniority,
        () => this.seniorityScore(talent, bonus.seniority!),
      ],
      [
        'keywords',
        !!bonus.keywords?.length,
        () => this.keywordSearch(talent, bonus.keywords!),
      ],
    ];
    ...
    return criteria
      ...
      .map(([key, , computeScore]) => ({
        key,
        score: computeScore(),
        weight: DEFAULT_BONUS_FILTER_WEIGHTS[key],
      }));
  }
  ...
  private computeLevelScore(
    ...
    if (ratio < 1) return ratio * MAX_SKILL_SCORE;
